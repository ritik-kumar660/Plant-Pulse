...
if (isset($plant['last_watered']) && $plant['last_watered']) {
    $last_watered = date('Y-m-d', strtotime($plant['last_watered']));
}
if (isset($plant['last_fertilized']) && $plant['last_fertilized']) {
    $last_fertilized = date('Y-m-d', strtotime($plant['last_fertilized']));
}
if (isset($plant['last_repotted']) && $plant['last_repotted']) {
    $last_repotted = date('Y-m-d', strtotime($plant['last_repotted']));
}
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="date" name="last_watered" class="form-control" value="<?php echo $last_watered; ?>">
                            </div>
                            <div>
                                <label class="form-label">Last Fertilized</label>
                                <input type="date" name="last_fertilized" class="form-control" value="<?php echo $last_fertilized; ?>">
                            </div>
                            <div>
                                <label class="form-label">Last Repotted</label>
                                <input type="date" name="last_repotted" class="form-control" value="<?php echo $last_repotted; ?>">
                            </div>
                        </div>
                        ...
                        <div class="border-t pt-6 mt-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="reminder_date" class="form-control" min="<?php echo $min_date; ?>">
                                </div>
                                <div>
                        ...
                        <div class="flex justify-end gap-4 mt-6">
                            <form method="POST" action="delete_plant.php" class="inline" onsubmit="return confirm('Are you sure you want to delete this plant? This action cannot be undone.');">
                                <input type="hidden" name="plant_id" value="<?php echo $plant_id; ?>">
                                <button type="submit" class="btn btn-danger px-6 py-2 rounded-lg font-semibold">
